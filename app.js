...
/* ───────── 存檔 ───────── */
const SAVE_KEY="chrono.save", SAVE_VERSION=2;
function defaultSave(){
  return { saveVersion:SAVE_VERSION, kitchenEra:"apocalypse",
    ...
    unlocked:{ actions:[], eras:["apocalypse"] }, settings:{ volume:0.8 }, stats:{ agings:0, discoveries:0 } };
}
function loadSave(){
  try{ const raw=localStorage.getItem(SAVE_KEY); if(!raw) return defaultSave();
    let s=JSON.parse(raw);
    if((s.saveVersion??0) < SAVE_VERSION) s = { ...defaultSave(), ...s, saveVersion:SAVE_VERSION }; // 破壞性改版→重置為新局
    return s;
...
function selectItem(uid){ selectedUid=uid; const it=S.inventory.find(x=>x.uid===uid); $("axis").value=daysToSlider(it?it.ageDays:0); sfxClick(); renderInventory(); renderSelected(); }
...
selectedUid = S.inventory[0] ? S.inventory[0].uid : null;
renderTicks(); renderAll();
